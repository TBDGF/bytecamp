...
func GetStudentCourse(c *gin.Context) {
	...
	db.NewDB().Select(&response.Data.CourseList,
		"select c.course_id, c.course_name, ts.teacher_id from course c left join teacher_schedule ts on c.course_id = ts.course_id left join student_schedule ss on c.course_id = ss.course_id where student_id = ?;", request.StudentID)
